...
int main(int argc, char** argv)
{
    ...
    //! [image_path_string]
    std::string pathToImageLeft("image_left/I1_%6d.png");
    std::string pathToImageRight("image_right/I2_%6d.png");
    cv::VideoCapture sequenceLeft(pathToImageLeft);
    cv::VideoCapture sequenceRight(pathToImageRight);
    int counter = 0;

    while(counter < 340){
        ++counter;
    if( !sequenceLeft.isOpened() || !sequenceRight.isOpened() )
    { std::cerr << "-- Unable to read image sequence!\n" << std::endl; return -1; }
    Mat left;
    Mat right;
    sequenceLeft >> left;
    sequenceRight >> right;
    ...
    */
    ...
    //cout<<"Time:"<<matching_time + filtering_time << "s" <<endl;
    Ttime += matching_time + filtering_time;
    //cout<<endl;
    /*
    ...
    */
    if(dst_path!="None")
    {
        ...
        ss << counter;
        std::string name = "result/resultDepth_" + ss.str() + ".png";
        imwrite(name,filtered_disp_vis);
    }
    ...
    cout << "Total time:" << Ttime << ", average time:"<< Ttime / 340.0 << endl;
    return 0;
}
